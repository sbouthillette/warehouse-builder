// api/access/verify.js — step 2 of the sign-in gate: checks the 6-digit code
// emailed by api/access/login.js against the `login_codes` table (see
...
// but now proven-self-serve — see api/access/login.js's file comment for
...
export default async function handler(req, res) {
  ...
  if (req.method === 'GET') {
    ...
    res.writeHead(302, { Location: `/api/access/login?next=${encodeURIComponent(next)}` });
    res.end();
    return;
  }
