...
// Shared by the Edge middleware (middleware.js) and the Node.js auth routes
// (api/auth/*.js) — both runtimes expose Web Crypto (crypto.subtle, btoa,
// atob, TextEncoder/TextDecoder) globally, so this file works unmodified in
// either place. Used for two things with the same primitives:
//   - the session cookie (proves a visitor signed in with an allowed
//     Google Workspace account)
//   - the OAuth "state" param (CSRF protection + carries the post-login
//     redirect target across the round trip to Google and back)
